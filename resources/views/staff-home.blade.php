  ...
  <div class="sidebar">
    ...
    <ul class="sidebar-list">
      ...
      <li class="sidebar-list-item" style="background-color: #f085c3; ">
        <a style="color:white" href="{{route('staff-home')}}">
    ...
    <div class="app-content">
      ...
      <div class="products-area-wrapper appointmentView">
        ...
        <div class="appointments-row" style="">
                ...
                <div class="parent-staff-app-items">
                    ...
                    <div class="product-cell-button" id="completeButton{{ $reservation->id }}">
                        @if ($reservation->status == 'Reserved')
                            <button style="color: white" class="confirm-button" onclick="confirmRent('{{ route('confirm.rent', ['id' => $reservation->id]) }}', '{{ $reservation->reservation_date }}')">Confirm Rent</button>
                        @elseif ($reservation->status == 'pending')
                            <button style="color: white" onclick="confirmMarkCompleted('{{ route('confirm.reservations', ['id' => $reservation->id]) }}')">Confirm Reservation</button>
                            <button style="background-color: darksalmon; color:white; margin-left:1em" onclick="confirmMarkDeclined('{{ route('decline.reservations', ['id' => $reservation->id]) }}')">Decline Reservation</button>
            ...
            <script>
                function confirmMarkCompleted(url) {
                    if (confirm('Are you sure you want to confirm this reservation?')) {
                        ...
                        // Redirect to mark the reservation as completed
            ...
            <!-- decline reservation -->
            <script>
                function confirmMarkDeclined(url) {
                    if (confirm('Are you sure you want to decline this reservation?')) {
                        // Redirect to mark the reservation as declined
                        window.location.href = url;
                    }
                }
            </script>
...
<script>
    function confirmRent(url, reservationDate) {
        var reservationDateObj = new Date(reservationDate);
        var currentDateUTC = new Date().toUTCString();
        var currentDateObj = new Date(currentDateUTC);
        var difference = reservationDateObj.getTime() - currentDateObj.getTime();

        // Convert the difference to days
        var differenceInDays = difference / (1000 * 3600 * 24);

        // Check if the difference is within the allowed confirmation period (from 1 day before to 3 days afterward)
        if (differenceInDays >= -1 && differenceInDays <= 3) {
            if (confirm("Are you sure you want to confirm this rent?")) {
                window.location.href = url;
            }
        } else {
            alert("Rent confirmation is available from 1 day before the reservation date to 3 days afterward.");
        }
    }
</script>
