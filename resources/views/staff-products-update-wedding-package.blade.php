...
<div class="staff-add">
          ...
          <div class="iphone-staff-add">
                ...
                <form action="{{ route('wp.update',['id' => $wpItems->id]) }}" class="form" method="POST" enctype="multipart/form-data">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="address">
                        ...
                        <div class="card-staff-add">
                            <address class="address">
                                    <label for="item">Product Name:</label><br>
                                        <input type="text" class="staff-name" id="item" name="item" value="{{$wpItems->item}}" required><br>
                                    <label for="description">Description:</label><br>
                                        <textarea style="text-transform:none"class="description-textarea" id="description" name="description" rows="5" cols="65" value="{{$wpItems->description}}" required>{{$wpItems->description}}</textarea><br>
                                    <label for="category">Category:</label><br>
                                        <input type="text" class="staff-name" id="category" name="category" value="{{$wpItems->category}}" required><br>

                                    <label for="price">Price:</label><br>
                                          <input type="number" class="staff-name" id="price" name="price" min="1" value="{{$wpItems->price}}" required><br>
                                <br><br>
                      ...
                      <div class="button-container">
                            <button class="staff-add-product --full" type="submit"> Update Wedding Package</button>
                      </div>
            </div>
            <br>
