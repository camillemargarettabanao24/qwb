...
class WeddingPackageManagerController extends Controller
{

// CUSTOMER SIDE
    public function manager_products_add_wedding_package(){ 
        ...
        return view('manager-products-add-wedding-package', compact('userManager'));
    }
    ...
    public function store(Request $request)
{
    ...
    // process the uploaded images and store them
        ...
        // Log activity
        ActivityLogs::create([
            'activity' => 'Added a Wedding Package',
            'user' => session('LoggedUserManager')->username
        ]);

        return redirect()->route('manager-products', $weddingPackage->id)
...
// STAFF WEDDING PACKAGE ADD, UPDATE, DELETE
    ...
    public function update_wedding_package_show($id)
    {
        $wpItems = WeddingPackage::findOrFail($id);
            ...
            return view('mananger-products-update-wedding-package', compact('wpItems'));
    }
    ...
    public function wp_update(Request $request, $id)
    {
        ...
        return redirect()->route('manager-products')->with('success', 'Wedding package updated successfully.');
    }
